...
//       props.onInputClick(roomType, originalImage);
//     } else {
//       navigate("/products", {
//         state: {
//           scrollToForm: true,
...
export default function AfterDraggableImagesProducts({ onInputClick }) {
  const navigate = useNavigate();
  const [fullscreenImage, setFullscreenImage] = useState(null);
  const [popupImage, setPopupImage] = useState(null);
  const [showOriginal, setShowOriginal] = useState({
  ...
  const handleInputRedirect = (roomType, originalImage) => {
    if (onInputClick) {
      onInputClick(roomType, originalImage);
    } else {
      // Ensure the image path is properly formatted
      const absoluteImagePath = new URL(originalImage, window.location.origin)
        .href;
      ...
      navigate("/FormAfter", {
        state: {
          scrollToForm: true,
          selectedRoom: roomType,
          originalImage: absoluteImagePath, // Use absolute path
        },
      });
    }
  };
  ...
  const RoomCard = ({ roomName, originalImage, transformedImage, roomKey }) => (
    <div className="max-w-[522px] m-auto w-full h-auto sm:min-h-auto flex flex-col gap-2">
      ...
      <div className="max-w-[522px] w-full h-auto rounded-[4px]">
        <DraggableImages
          imageLeft={originalImage}
          imageRight={transformedImage}
        />
      </div>
      <div className="max-w-[520px] min-h-[57px] flex justify-between items-center">
        <div
